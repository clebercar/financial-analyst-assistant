...
Documento de governanca obrigatorio do Datathon Fase 5. Mapeia as ameacas
relevantes ao nosso assistente de analista financeiro e a mitigacao
implementada (ou planejada como roadmap).

## Tabela de mapeamento

| ID    | Ameaca                            | Risco no nosso sistema                                                                 | Mitigacao implementada                                                                                       | Status                                                |
|-------|-----------------------------------|----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------|-------------------------------------------------------|
...
| LLM10 | Unbounded Consumption             | Custo Gemini pode escalar com requests massivos; sem rate limit por IP                 | `max_tokens` configurado no Gemini + limite de 4096 chars no input. Falta `slowapi` para rate limit por IP   | Parcial - rate limit por IP fica no roadmap pos-MVP   |

## Ameacas nao cobertas no MVP

Decisao consciente para caber em 9 dias de implementacao. Documentado no
`docs/SYSTEM_CARD.md` Secao 4 (trade-offs).

| ID    | Ameaca                       | Por que nao cobrimos                                                              | Plano                                                                  |
|-------|------------------------------|-----------------------------------------------------------------------------------|------------------------------------------------------------------------|
| LLM03 | Supply Chain                 | Auditoria de pesos de modelos terceiros (Gemini, embeddings) e custosa            | Pos-MVP: pinning de versoes + scan automatico via `pip-audit`          |
| LLM04 | Data Poisoning               | RAG indexa apenas filings publicos da SEC (fonte autoritaria)                     | Pos-MVP: hash de cada doc indexado + re-verificacao periodica          |
| LLM05 | Improper Output Handling     | Output e texto puro mostrado ao usuario; nao executamos como codigo               | Continua suficiente para MVP                                           |
| LLM08 | Vector Weaknesses            | ChromaDB local; risco baixo no MVP                                                | Pos-MVP: rodar `prompt-injection-bench` contra a colecao               |
...
- Decisoes de design e trade-offs: `docs/SYSTEM_CARD.md` Secao 4
