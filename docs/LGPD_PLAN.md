...
Lei 13.709/2018 (Lei Geral de Protecao de Dados Pessoais). Documento de
governanca obrigatorio do Datathon Fase 5.
...
- **Endpoint `/chat`:** input do usuario (texto livre).
  ...
  - **Tratamento:** logs sao truncados em 120 chars (ver
...
- **Tracing Langfuse:** input + output armazenados em servidor externo (UE).
  - **Base legal:** legitimo interesse + consentimento implicito (uso
    explicito de servico educacional).
...
|------------------------------------|---------------------------------------------------------------------------------|
| Acesso (Art. 18 I)                 | Usuario envia email para DPO solicitando dump dos logs com seu IP               |
| Anonimizacao (Art. 18 IV)          | Presidio aplicado automaticamente em outputs do `/chat`                         |
| Eliminacao (Art. 18 VI)            | DPO executa script `python -m scripts.delete_user_logs --ip X` (a implementar)  |
...
- **Inputs persistidos para retraining:** 90 dias, sanitizados (a implementar
  como pipeline batch).
...
Sao terceiros que processam dados em nosso nome. Cada um tem DPA padrao
publico do fornecedor.
...
1. **Deteccao:** alerta no Prometheus quando rate de erro > 1% ou quando
   contador `chat_requests_total{status="blocked_input"}` cresce de forma
   anomala.
...
4. **Notificacao:** se PII vazou de fato, notificar ANPD em 72h conforme
   Art. 48 e titulares afetados conforme Art. 48 paragrafo 1.
5. **Remediacao:** patch + retraining se houver suspeita de data poisoning.

## 9. Atualizacoes desse plano

Esse plano sera revisto a cada nova versao maior do sistema (mudanca de
modelo, novo operador externo, novo endpoint que coleta dado novo).

Ultima revisao: Dia 8 do Datathon Fase 5 (2026-05-03).
